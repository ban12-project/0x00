...
export default class Jpeg implements Processor {
  ...
  private tags: Record<
    | (typeof Jpeg.IFDMap)[number]['name']
    | (typeof Jpeg.MakerNoteIFDMap)[number]['name'],
    string | number
  > = {}

  constructor(view: DataView) {
    if (!Jpeg.isValid(view)) throw new Error('not a valid JPEG')

    this.view = view
  }

  static isValid(view: DataView) {
    return view.getUint16(0) === Jpeg.SOI
  }

  read() {
    // The first two bytes (offset 0-1) was the SOI marker
    this.offset += 2
    while (this.marker !== Jpeg.SOS) {
      this.marker = this.view.getUint16(this.offset)
      const size = this.view.getUint16(this.offset + 2)
      if (
        this.marker === Jpeg.APP1 &&
        this.view.getUint32(this.offset + 4) === Jpeg.EXIF
      ) {
        ...
        if (this.view.getUint16(this.offset) === Jpeg.LITTLE_ENDIAN)
          this.isLittleEndian = true
        if (this.view.getUint16(this.offset) === Jpeg.BIG_ENDIAN)
        ...
        for (
          ...
          if (tagId === Jpeg.TAG_ID_EXIF_SUB_IFD_POINTER) {
            // It's a LONG, so 4 bytes must be read
            exifSubIfdOffset = this.view.getUint32(i + 8, this.isLittleEndian)
          }

          if (!Jpeg.IFDMap[tagId]) continue

          const name = Jpeg.IFDMap[tagId].name
  ...
  private processAppleMakerNotes(baseOffset: number) {
    ...
    for (let i = 0; i < numEntries; i++) {
      ...
      if (Jpeg.MakerNoteIFDMap[tagId]) {
        const key = Jpeg.MakerNoteIFDMap[tagId].name
